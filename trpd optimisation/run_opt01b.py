...
result_file = "run_opt01b_results.txt"
...
# bounds = [(0, 2*np.pi), (0, np.pi), (0, 2*np.pi), (-2, 2), (-2, log_k12max), (-2, log_k21max)]
bounds = [(-2*np.pi,4*np.pi), (-np.pi, 2*np.pi), (-2*np.pi,4*np.pi), (-2, 2), (-2, log_k12max), (-2, log_k21max)]
...
for n in range(repeats):
    ...
    x0 = np.concatenate((oriD, np.array([lb + np.random.rand() * (ub - lb) for ub, lb in [(0,2), (1,5), (1,5)]])))
