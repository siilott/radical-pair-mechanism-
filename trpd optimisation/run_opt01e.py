...
result_file = "run_opt01e_results.txt"
...
# bounds = [(0, 2*np.pi), (0, np.pi), (0, 2*np.pi), (0, 2*R0), (-R0, R0), (-R0,R0), (0, 2), (-0.3, 0), (0.2, 1.5)]
bounds = [(-2*np.pi,4*np.pi), (-np.pi, 2*np.pi), (-2*np.pi,4*np.pi), (0, 2*R0), (-R0, R0), (-R0,R0), (0, 2), (-0.3, 0), (0.2, 1.5)]

repeats = 30
for n in range(repeats):
    # x0 = np.array([lb + np.random.rand() * (ub - lb) for ub, lb in bounds])
    x0 = np.concatenate((oriD, [R0, np.random.randn(), np.random.randn()], np.array([lb + np.random.rand() * (ub - lb) for ub, lb in [(0,2), (-0.1,0), (0.2,0.5)]])))
